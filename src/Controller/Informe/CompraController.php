...
{
    /**
     * @Route("/informe/compra/lista/", name="informe_compra_lista")
     */
    public function lista(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($form->get('btnExcel')->isClicked()) {
                $arMovimiento = $em->getRepository(Movimiento::class)->informeCompra($empresa);
                $this->excel($arMovimiento);
            }
        }
        $arMovimientos = $paginator->paginate($em->getRepository(Movimiento::class)->informeCompra($empresa), $request->query->getInt('page', 1), 30);
        return $this->render('informe/compra/compra.html.twig', [
    ...
    /**
     * @Route("/informe/compradetalle/lista/", name="informe_compradetalle_lista")
     */
    public function listaDetalle(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($form->get('btnExcel')->isClicked()) {
                $arMovimientoDetalles = $em->getRepository(MovimientoDetalle::class)->informeCompra($empresa)->getQuery()->execute();
                $this->excelDetalle($arMovimientoDetalles);
            }
        }
        $arMovimientoDetalles = $paginator->paginate($em->getRepository(MovimientoDetalle::class)->informeCompra($empresa), $request->query->getInt('page', 1), 30);
        return $this->render('informe/compra/compraDetalle.html.twig', [
    ...
    public function excelDetalle($arMovimientos)
    {
        ...
        $libro->setActiveSheetIndex(0);
        header('Content-Type: application/vnd.ms-excel');
        header("Content-Disposition: attachment;filename=comprasDetalle.xls");
        header('Cache-Control: max-age=0');
        $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($libro, 'Xls');
        $writer->save('php://output');
        $writer = new Xlsx($libro);
